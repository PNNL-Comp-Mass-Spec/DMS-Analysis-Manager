...
namespace AnalysisManagerExtractionPlugin
{
    ...
    public class clsPeptideExtractWrapper
    {
        ...
        private void m_ExtractTools_CurrentProgress(double fractionDone)
        {
            ...
            if (m_DebugLevel > 3 && System.DateTime.UtcNow.Subtract(dtLastLogTime).TotalSeconds >= MIN_LOG_INTERVAL_SECONDS)
            {
                // Over MIN_LOG_INTERVAL_SECONDS seconds has elapsed; update the log file
                blnUpdateLog = true;
            }
            else if (m_DebugLevel >= 1 && System.DateTime.UtcNow.Subtract(dtLastLogTime).TotalSeconds >= MAX_LOG_INTERVAL_SECONDS)
            {
