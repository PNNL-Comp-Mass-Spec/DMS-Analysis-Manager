...
#Region "Methods"
    ...
    Protected Function ConvertMODPlusResultsToTxt(<Out()> ByRef strFilteredMODPlusResultsFilePath As String, ByVal keepAllResults As Boolean) As IJobParams.CloseOutType
        ...
        Const isModPlus = True

        Return ConvertMODaOrMODPlusResultsToTxt(fdrThreshold, decoyPrefix, isModPlus, strFilteredMODPlusResultsFilePath, keepAllResults)
    End Function

    Protected Function ConvertMODaOrMODPlusResultsToTxt(
      fdrThreshold As Single,
      ...
      ByVal keepAllResults As Boolean) As IJobParams.CloseOutType
        ...
        Try
            ...
            Dim modxProgJarName As String
            Dim modxFilterJarName As String
            ...
            Else
                ...
                Dim fiFastaFile = New FileInfo(mGeneratedFastaFilePath)
                ...
                For Each decoyPrefix In decoyPrefixes
                    ...
                    If fractionDecoy * 100 >= MINIMUM_PERCENT_DECOY Then
            ...
            Dim resultsFilePath = Path.Combine(m_WorkDir, m_Dataset & fileNameSuffix)
            ...
            If m_DebugLevel >= 2 Then
                clsLogTools.WriteLog(clsLogTools.LoggerTypes.LogFile, clsLogTools.LogLevels.INFO, "Filtering MODa/MODPlus Results with FDR threshold " & fdrThreshold.ToString("0.00"))
            End If

            Const javaMemorySize = 1000
            ...
            Dim modxProgLoc = DetermineProgramLocation(toolName, toolName & "ProgLoc", modxProgJarName)

            Dim fiMODx = New FileInfo(modxProgLoc)

            'Set up and execute a program runner to run anal_moda.jar or tda_plus.jar
            Dim cmdStr = " -Xmx" & javaMemorySize.ToString & "M -jar " & Path.Combine(fiMODx.DirectoryName, modxFilterJarName)
            cmdStr &= " -i " & resultsFilePath

            If Not isModPlus Then
            ...
            If Not blnSuccess Then
                Dim msg = "Error parsing and filtering " & toolName & " results"
                clsLogTools.WriteLog(clsLogTools.LoggerTypes.LogFile, clsLogTools.LogLevels.ERROR, msg & ", job " & m_JobNum)
                m_message = clsGlobal.AppendToComment(m_message, msg)

                If progRunner.ExitCode <> 0 Then
                    clsLogTools.WriteLog(clsLogTools.LoggerTypes.LogFile, clsLogTools.LogLevels.WARN, modxFilterJarName & " returned a non-zero exit code: " & progRunner.ExitCode.ToString)
                Else
            ...
            Dim fiFilteredResultsFilePath = New FileInfo(Path.ChangeExtension(resultsFilePath, ".id.txt"))
    ...
    Private Function RunPhrpForMODa(ByVal strFilteredMODaResultsFilePath As String) As IJobParams.CloseOutType
        ...
        Try
            ...
            Try
                ...
                Const CreateFirstHitsFile = True
    ...
    Protected Function ValidatePHRPResultMassErrors(ByVal strInputFilePath As String, ByVal eResultType As clsPHRPReader.ePeptideHitResultType, ByVal strSearchEngineParamFileName As String) As Boolean
        ...
        Try
            ...
            If Not blnSuccess Then
                ...
                If Not blnSuccess Then
                    If String.IsNullOrWhiteSpace(oValidator.ErrorMessage) Then
