...
fraction <- {$TARGET_DATASET_FRACTION}
...
def.baseline <- {$USE_EXISTING_BASELINE}
...
alt.baseline <- "{$NEW_BASELINE_DATASET_INFO_FILENAME}"
...
QC_ART <- function(dataset,fraction,def.baseline,alt.baseline,MASIC,SMAQC){
  default.baseline <- "{$EXISTING_BASELINE_CSV_NAME}"
  ...
  if(!def.baseline) {
    ...
    write.table(baseline.data, "{$NEW_BASELINE_DATA_FILENAME}", sep=",", row.names=FALSE, col.names=TRUE, na="") 
  }
...
write.table(distances, "{$TARGET_DATASET_RESULTS}", sep="\t", row.names=TRUE, col.names=FALSE, na="", quote=FALSE) 
