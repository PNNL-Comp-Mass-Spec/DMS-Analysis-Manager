...
Public Class clsAnalysisResourcesLipidMapSearch
	...
	Protected Function RetrieveDatasetAndPeaksFile(ByVal strDatasetName As String, ByVal strDatasetFolderPath As String, ByVal strDatasetFolderPathArchive As String, ByVal strDeconToolsFolderName As String) As Boolean
		...
		strFileToFind = strDatasetName & DECONTOOLS_PEAKS_FILE_SUFFIX
		If Not CopyFileToWorkDir(strFileToFind, System.IO.Path.Combine(strDatasetFolderPath, strDeconToolsFolderName), m_WorkingDir, clsLogTools.LogLevels.INFO) Then
			' _Peaks.txt file not found on the storage server; try the archive
			If Not CopyFileToWorkDir(strFileToFind, System.IO.Path.Combine(strDatasetFolderPathArchive, strDeconToolsFolderName), m_WorkingDir, clsLogTools.LogLevels.INFO) Then
				' _Peaks.txt file still not found; this is OK
			End If
		End If
