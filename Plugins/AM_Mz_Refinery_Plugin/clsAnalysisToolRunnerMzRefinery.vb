...
#Region "Constants and Enums"
	...
	Protected Const PROGRESS_PCT_MSGFPLUS_COMPLETE As Single = 90
	Protected Const PROGRESS_PCT_MZREFINERY_COMPLETE As Single = 98
	Protected Const PROGRESS_PCT_COMPLETE As Single = 99

	Protected Const MzRefinery_CONSOLE_OUTPUT As String = "MzRefinery_ConsoleOutput.txt"
#End Region
...
#Region "Methods"
	...
	Public Overrides Function RunTool() As IJobParams.CloseOutType
		...
		Try
			...
			If fiMzRefResultsFile.Exists Then
				blnSuccess = PostProcessMzRefineryResults(fiMzRefResultsFile)
	...
	Protected Function RunMSGFPlus(
	  ...
	  <Out()> ByRef fiMSGFPlusResults As FileInfo) As IJobParams.CloseOutType
		...
		Dim strParameterFilePath As String = m_jobParams.GetJobParameter("MzRefParamFile", String.Empty)
		...
		Dim blnSuccess = StartMSGFPlusLocal(javaExePath, strSearchEngineName, CmdStr)
	...
	Private Sub MonitorProgress()
		...
		If DateTime.UtcNow.Subtract(dtLastConsoleOutputParse).TotalSeconds >= 30 Then
			...
			ElseIf mRunningMzRefinery Then
				ParseMzRefineryConsoleOutputFile(Path.Combine(m_WorkDir, MzRefinery_CONSOLE_OUTPUT))
			End If
	...
	Private Sub ParseMSGFPlusConsoleOutputFile(ByVal workingDirectory As String)
		...
		Try
			If Not mMSGFDBUtils Is Nothing Then
				...
				UpdateProgress(PROGRESS_PCT_STARTING, PROGRESS_PCT_MSGFPLUS_COMPLETE, msgfPlusProgress)
			End If
	...
	''' <summary>
	''' Parse the MzRefinery console output file to track the search progress
	''' </summary>
	...
	Private Sub ParseMzRefineryConsoleOutputFile(ByVal strConsoleOutputFilePath As String)
		...
		Const REGEX_MZ_REFINERY_PROGRESS As String = "(\d+)% complete"
		Static reCheckProgress As New Regex(REGEX_MZ_REFINERY_PROGRESS, Text.RegularExpressions.RegexOptions.Compiled Or Text.RegularExpressions.RegexOptions.IgnoreCase)
		...
		Try
			...
			UpdateProgress(PROGRESS_PCT_MSGFPLUS_COMPLETE, PROGRESS_PCT_MZREFINERY_COMPLETE, progressComplete)
	...
	Private Function PostProcessMzRefineryResults(ByVal fiMzRefResultsFile As FileInfo) As Boolean
		...
		Try
			...
			m_IonicZipTools.GZipFile(fiMzRefResultsFile.FullName, True)
	...
	Protected Function StartMzRefinery(ByVal fiMSGFPlusResults As FileInfo) As Boolean
		...
		With CmdRunner
			...
			.WriteConsoleOutputToFile = True
			.ConsoleOutputFilePath = Path.Combine(m_WorkDir, MzRefinery_CONSOLE_OUTPUT)
		End With

		m_progress = PROGRESS_PCT_MSGFPLUS_COMPLETE
		...
		m_StatusTools.UpdateAndWrite(m_progress)
		If m_DebugLevel >= 3 Then
			clsLogTools.WriteLog(clsLogTools.LoggerTypes.LogFile, clsLogTools.LogLevels.DEBUG, "MzRefinery Search Complete")
		End If
