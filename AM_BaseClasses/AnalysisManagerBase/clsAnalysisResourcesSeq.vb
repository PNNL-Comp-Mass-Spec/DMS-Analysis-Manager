...
Public Class clsAnalysisResourcesSeq
    ...
    ''	MyBase.New(mgrParams, jobParams, logger)

    ''End Sub

	Protected Overrides Function RetrieveOrgDB(ByVal OrgDBName As String, ByVal OrgDBPath As String) As Boolean
		...
		If Not MyBase.RetrieveOrgDB(OrgDBName, OrgDBPath) Then Return False
		...
		If VerifyDatabase(OrgDBName, OrgDBPath) Then
			Return True
		Else
			Return False
		End If
	...
	Private Function VerifyDatabase(ByVal OrgDBName As String, ByVal OrgDBPath As String) As Boolean
		...
		'Verifies the database required by the job is installed on the host and distributed to
	...
	Private Function VerifyRemoteDatabase(ByVal DbName As String, ByVal SourcePath As String, _
	 ByVal DestPath As String) As Boolean
		...
		Dim DestTime As String
		Try
			If File.Exists(DestFile) Then
				'File was found on node, compare date and time with copy on master
				SourceFI = New FileInfo(SourceFile)
				DestFI = New FileInfo(DestFile)
				'Linux doesn't support fractional seconds, so we'll have to create string versions of the file time stamp
				'	that use whole seconds
				SourceTime = SourceFI.LastWriteTime.ToString("MM/dd/yyyy HH:mm:ss")
				DestTime = DestFI.LastWriteTime.ToString("MM/dd/yyyy HH:mm:ss")
				'Compare file sizes
				If SourceFI.Length <> DestFI.Length Then CopyNeeded = True
				'Compare file modified dates
				If CDate(SourceTime) <> CDate(DestTime) Then CopyNeeded = True
			Else
	...
	'Private Function SetBioworksVersion(ByVal InpVersion As String) As IGenerateFile.ParamFileType
